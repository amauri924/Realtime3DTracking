...
class LoadImages:  # for inference
    ...
    def __next__(self):
        ...
        return path, img, img0, self.cap, calib
...
#                labels[:, 6] = (ratioh * h * x[:, 6] + padh)-labels[:, 2]
        ...
        labels_out = torch.zeros((nL, 6+3+5))
        ...
        if random.random() > 0.5 and self.depth_aug:
            augmented_roi=rois_augmentation_for_depth(labels_out,0.1,0.)
        else:
...
class LoadImagesAndLabels_display(Dataset):  # for training/testing
    def __init__(self, path, img_size=416, batch_size=16, augment=False, rect=True, image_weights=False, depth_aug=False):
        with open(path, 'r') as f:
            img_files = f.read().splitlines()
            self.img_files = np.array([x for x in img_files if os.path.splitext(x)[-1].lower() in img_formats])
...
#                labels[:, 6] = (ratioh * h * x[:, 6] + padh)-labels[:, 2]
        ...
        if random.random() > 0.5 and self.depth_aug:
            augmented_roi=rois_augmentation_for_depth(labels_out,0.05,0.02)
        else:
            augmented_roi=labels_out.clone()
...
def rois_augmentation_for_depth(targets,sigma_shape,sigma_center):
    ...
    for i in range(len(targets)):
        ...
        h_var=random.random()*sigma_shape 
        w_var=random.random()*sigma_shape
        x_var=random.random()*sigma_center
        y_var=random.random()*sigma_center
