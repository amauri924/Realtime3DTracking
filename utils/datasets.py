...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=416, batch_size=16, augment=False, rect=True, image_weights=False):
        ...
        self.imgs = [None] * n
        self.labels = [None] * n
        preload_labels = self.mosaic
        if preload_labels:
            self.labels = [np.zeros((0, 8))] * n
            iter = self.label_files if n > 10 else self.label_files
            extract_bounding_boxes = False
            for i, file in enumerate(tqdm(iter)):
                try:
...
def random_perspective(img, targets=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0, border=(0, 0)):
    ...
    if n:
        # warp points
        xy = np.ones((n * 5, 3))
        xy[:, :2] = targets[:, [1, 2, 3, 4, 1, 4, 3, 2,5,6]].reshape(n * 5, 2)  # x1y1, x2y2, x1y2, x2y1 , xcyc
        xy = xy @ M.T  # transform
        if perspective:
            xy = (xy[:, :2] / xy[:, 2:3]).reshape(n, 10)  # rescale
        else:  # affine
            xy = xy[:, :2].reshape(n, 10)
...
def plot_images(images, targets, paths=None, fname='images.jpg', names=None, max_size=640, max_subplots=16):
    ...
    for i, img in enumerate(images):
        ...
        if len(targets) > 0:
            ...
            gt = image_targets.shape[1] == 9  # ground truth if no conf column
