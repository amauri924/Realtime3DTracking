...
def train(data_cfg,img_size,epochs,device,world_size,batch_size=16,accumulate=1):
...
#    device = torch_utils.select_device()
    ...
    test_results=test(model.module if type(model) is torch.nn.parallel.DistributedDataParallel else model,
                              device,batch_size,test_path,img_size).item()
...
#    scheduler = lr_scheduler.ReduceLROnPlateau(optimizer, 'min',verbose=True,min_lr=1e-7)
    ...
    scheduler= lr_scheduler.CosineAnnealingWarmRestarts(optimizer,T_0=50,eta_min=1e-7)
...
#    if torch.cuda.device_count() > 1:
    dist.init_process_group(backend='gloo',  # 'distributed backend'
                            ...
                            rank=device)  # distributed training node rank
...
def example(rank, world_size):
    train('data/GTA_3dcent.data',416,200,rank,world_size)
