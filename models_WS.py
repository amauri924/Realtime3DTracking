...
from torch.nn import Conv2d as Conv2d_WS
...
class BatchNorm2d(nn.BatchNorm2d):
    ...
    def __init__(self, filters):
        super(BatchNorm2d, self).__init__(filters)
        self.bn=nn.GroupNorm(16,filters)
    ...
    def init_module(self,module_defs):
        ...
        for i, module_def in enumerate(module_defs):
            ...
            if module_def['type'] == 'convolutional':
                ...
                if bn:
                    modules.add_module('conv_%d' % i, Conv2d_WS(in_channels=output_filters[-1],
...
#        self.dep = nn.Sequential(
        self.conv1=Conv2d_WS(num_channel, num_channel,
                  kernel_size=3, stride=1, padding=0, bias=False)
        self.bn1=BatchNorm2d(num_channel)
        self.relu=nn.ReLU(inplace=True)
        self.conv2=Conv2d_WS(num_channel, num_channel,
                  kernel_size=3, stride=1, padding=0, bias=False)
        self.bn2=BatchNorm2d(num_channel)

        self.conv3=Conv2d_WS(num_channel, num_channel,
                  kernel_size=3, stride=1, padding=0, bias=False)
        self.bn3=BatchNorm2d(num_channel)
...
#        self.dep = nn.Sequential(
        self.conv1=Conv2d_WS(num_channel, num_channel*2,
                  kernel_size=3, stride=1, padding=0, bias=False)
        self.bn1=BatchNorm2d(num_channel*2)
        self.relu=nn.ReLU(inplace=True)
        self.conv2=Conv2d_WS(num_channel*2, num_channel,
                  kernel_size=3, stride=1, padding=0, bias=False)
        self.bn2=BatchNorm2d(num_channel)

        self.conv3=Conv2d_WS(num_channel, num_channel*2,
                  kernel_size=3, stride=1, padding=0, bias=False)
        self.bn3=BatchNorm2d(num_channel*2)
...
#        _ = load_darknet_weights(self, "weights/" + 'darknet53.conv.74')
    ...
    def forward(self, x, var=None,targets=None,conf_thres=0,nms_thres=0,testing=False,detect=False):
        ...
        if not self.training and not self.transfer:
            ...
            divider2=torch.sqrt(orientation_pred[:,6:7]*orientation_pred[:,6:7]+orientation_pred[:,7:8]*orientation_pred[:,7:8])
