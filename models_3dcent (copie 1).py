...
class RoiAlign(nn.Module):
    ...
    def foward(self,features,filtered_roi):
        if features.type()==filtered_roi[0].type():
            box=torch.zeros([filtered_roi[0].shape[0], 5], dtype=torch.float32).cuda()
        else:
            box=torch.zeros([filtered_roi[0].shape[0], 5], dtype=torch.float16).cuda()
        box[:,1:] = filtered_roi[0][:, :4].detach()
...
#        else:
        ...
        io=torch.cat(io, 1)
        time_NMS=time.time()
        filtered_roi = non_max_suppression(io, conf_thres=0.001, nms_thres=0.5)
        time_NMS=time.time()-time_NMS
        
#        pooled_features=roi_align(layer_outputs[74], box, (7,7), spatial_scale=1.0/32.0)
#        if layer_outputs[74].type()==filtered_roi[0].type():
#            box=torch.zeros([filtered_roi[0].shape[0], 5], dtype=torch.float32).cuda()
#        else:
#            box=torch.zeros([filtered_roi[0].shape[0], 5], dtype=torch.float16).cuda()
#        box[:,1:] = filtered_roi[0][:, :4].detach()
#        
##        box=box.type(torch.cuda.HalfTensor)
##        box=Variable(box)
##        image_features=layer_outputs[74]
#        
#        pooled_features=roi_align(layer_outputs[74], box, (7,7), spatial_scale=1.0/32.0)
