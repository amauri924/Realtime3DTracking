...
import math
import tqdm
import test_3dcent as test  # import test.py to get mAP after each epoch
from models_3dcent import *
from utils.datasets_3dcent import *
...
#            for test_step in range(10):
            ...
            t_pred=time.time()
            pred,pred_center = model(imgs,targets=targets)
...
#                f.write("step %i\n"%i)
            ...
            # Compute loss
            try:
                loss, loss_items = compute_loss(pred,pred_center, targets, model, giou_loss=not opt.xywh)
            except:
                print("########### error during loss computing #######################")
                continue
