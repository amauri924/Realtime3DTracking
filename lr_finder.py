...
def train(
        ...
        opt,
        img_size=416,
        epochs=1,  # 500200 batches at bs 16, 117263 images = 273 epochs
        batch_size=16,
    ...
    Yolo_param=model.Yolov3.parameters()
    center_param=model.center_prediction.parameters()
    depth_param=model.depth_pred.parameters()


    # Optimizer
    
#    optimizer = optim.Adam(Yolo_param,
#                       lr=1e-4,  weight_decay=3e-6)
    
    optimizer = optim.Adam(
    [
        {'params': Yolo_param, 'lr': 1e-4, 'weight_decay':3e-6},
        {'params': center_param, 'lr': 1e-4, 'weight_decay':3e-6},
        {'params': depth_param, 'lr': 1e-4, 'weight_decay':3e-6},
    ])
...
#    model = torch.nn.parallel.DistributedDataParallel(model,device_ids=[rank],find_unused_parameters = True)
    ...
    lr_values=np.geomspace(1e-8,1,int(nb/accumulate))
...
#            model=setup_net_transfert(model)
    ...
    for i, (imgs, targets, paths, _,calib,pixel_to_normalized_resized) in enumerate(tqdm(dataloader)):
        ...
        batch_loss.append(loss_items[-2].item())
        ...
        if (i + 1) % accumulate == 0 or (i + 1) == nb:
            new_lr=optimizer.param_groups[2]['lr']
            with open("lr.txt","a") as f:
                f.write("LR:"+str(new_lr)+'\n')
            
            batch_loss=np.mean(np.array(batch_loss))
            optimizer.param_groups[2]['lr']=lr_values[int((i+1)/accumulate)]
            ...
            with open("it_loss.txt","a") as f:
                f.write(str(smoothed_loss)+'\n')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--notest', default=True, help='only test final epoch')
