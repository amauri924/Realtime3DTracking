...
#            logfile.write("nb GPU : %i\n"%torch.cuda.device_count())
    ...
    real_lr_list=[]
    real_loss_orient_list=[]
    real_loss_depth_list=[]
    real_loss_center_list=[]


    lr_values=np.geomspace(1e-8,1e-2,int(nb/accumulate)+1)
    ...
    for i, (imgs, targets, paths, _,calib,pixel_to_normalized_resized,augmented_roi) in enumerate(tqdm(dataloader)):
        for param_group in optimizer.param_groups:
            param_group['lr'] = lr_values[int(i/accumulate)]
        if len(targets)>0:
            # if opt.depth_aug:
            ...
            lr_list.append(lr_values[int(i/accumulate)])
            loss_orient_list.append(loss_items[-3].item())
            loss_depth_list.append(loss_items[-5].item())
            loss_center_list.append(loss_items[-6].item())

        # Accumulate gradient for x batches before optimizing
        if (i + 1) % accumulate == 0 or (i + 1) == nb:

            real_lr_list.append(np.mean(np.array(lr_list)))
            real_loss_orient_list.append(np.mean(np.array(loss_orient_list)))
            real_loss_depth_list.append(np.mean(np.array(loss_depth_list)))
            real_loss_center_list.append(np.mean(np.array(loss_center_list)))
    ...
    if rank == 0:
        ...
        ax1.plot(real_lr_list,real_loss_orient_list)
        ax2.plot(real_lr_list,real_loss_depth_list)
        ax3.plot(real_lr_list,real_loss_center_list)
        
        fig.savefig("LR_evolution.png",dpi=1500)
