...
#            nf = int(model.module_defs[model.yolo_layers[0] - 1]['filters'])  # yolo layer size (i.e. 255)
    ...
    scheduler = optim.lr_scheduler.OneCycleLR(optimizer, 7e-4, epochs=epochs, steps_per_epoch=int(len(dataloader)/accumulate)+1,pct_start=0.08)
...
#                f.write("LR:"+str(optimizer.param_groups[0]['lr'])+'\n')
            ...
            if not opt.notest and epoch%5==0:
